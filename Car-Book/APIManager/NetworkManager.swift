...
//    }()
    ...
    class func sendRequest(urlPath:String, type:HTTPMethod, parms:[String:Any]?, success:((_ responseObject:[String : Any],_ suces:Bool)->Void)!, faliure:((_ errMsg:String,_ errCode:Int)->Void)!) {
        ...
        /*qvar request = URLRequest.init(url:URL.init(string: urlPath.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!)!)
...
//        request.allHTTPHeaderFields = header
        ...
        if User.curentUser?.token ?? "" != "" {
            request.addValue(User.curentUser?.token ?? "", forHTTPHeaderField:"Authorization" )
        }
        ...
        let task = session.dataTask(with: request as URLRequest) { data, response, error in
            ...
            do {
                ...
                success(json,true)
                    ...
                    faliure("\(String(describing: jsonStr))" , Constant.APIResponseCodes.statusCodeInternalServerError)
                }
...
//     }
    ...
    static func getJSONXAPiKey() -> [String:String] {
        return ["Content-Type": "application/x-www-form-urlencoded","Accept":"application/json"]
                //,"Authorization":User.curentUser?.token ?? ""]
//        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
//        request.addValue(, forHTTPHeaderField: )
//        request.addValue("Authorization", forHTTPHeaderField: User.curentUser?.token ?? "")
    }
    ...
    class func uploadTo(_ isImg:Bool,imgVw:Data?,urlPath:String, paramName:String , param: [String : Any] , fileType:String ,success:((_ fileName:[String: Any]?,_ suces:Bool)->Void)!, faliure:((_ errCode:Int,_ errMsg:String)->Void)!) {
        let token = User.curentUser?.token ?? ""
